...
class BABSBootstrap(BABS):
    ...
    def babs_bootstrap(
        ...
        # not to track the resources recorded for each submitted job:
        gitignore_file.write('\n' + 'code/job_resources.csv')
        ...
        # Generate the scripts and template of the job that records the resources
        # each job used; `babs submit` submits it after each job array:
        print('\nGenerating the scripts that record the resources jobs use...')
        container.generate_bash_sacct_job(self, system)
        container.generate_sacct_job_submit_template(
            op.join(self.analysis_path, 'code', 'submit_sacct_job_template.yaml'),
            self,
            system,
        )
        ...
        if self.pipeline is not None:
            ...
            template_paths = [
                ...
                'code/submit_sacct_job_template.yaml',
                'code/sacct_job.sh',
                'code/sacct_job.py',
            ]
            for i, step in enumerate(self.pipeline):
